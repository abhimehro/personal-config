...
2. Set the behavior to **AutoSlow when not in use** (slows the process to near-0%
   CPU after it has been idle/background for the grace period).
3. For the always-background system daemons below, you can instead use
   **Slow down this process** outright since you never interact with them.
...
- `Windscribe` / `windscribe` -- VPN; throttling can drop the tunnel (your
  hard gate depends on it).
