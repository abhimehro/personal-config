...
EOF
	...
	if [[ $has_recs == false ]]; then
		echo '            <li class="empty-state" style="color: #155724; background-color: #d4edda; border-color: #c3e6cb; padding: 15px; border-radius: 4px; display: flex; align-items: center; gap: 10px;"><span role="status" style="display: flex; align-items: center; gap: 10px;"><span aria-hidden="true" style="font-size: 1.2em;">✅</span> <span>System is running smoothly. No specific recommendations at this time.</span></span></li>' >>"$report_file"
	fi
...
EOF
		...
		cat >>"$report_file" <<EOF
        <p>Last optimization run: ${last_run}</p>
    </section>
EOF
	fi
...
# Execute main function only when run directly, not when sourced
