...
<body>
    <div class="container">
        ...
        <div class="metrics-grid">
            <div class="metric-card success" role="group" aria-labelledby="label-health-score">
                <div class="metric-value">${current_health}</div>
                <div class="metric-label" id="label-health-score">Health Score</div>
            </div>
EOF
	...
	if command -v jq >/dev/null 2>&1 && [[ -f $metrics_report ]]; then
		...
		cat >>"$dashboard_file" <<EOF
            <div class="metric-card" role="group" aria-labelledby="label-performance-score">
                <div class="metric-value">${avg_performance}</div>
                <div class="metric-label" id="label-performance-score">Performance Score</div>
            </div>
            <div class="metric-card $([ "${avg_disk:-0}" -gt 85 ] && echo "warning" || echo "success")" role="group" aria-labelledby="label-disk-usage">
                <div class="metric-value">${avg_disk}%</div>
                <div class="metric-label" id="label-disk-usage">Disk Usage</div>
            </div>
            <div class="metric-card $([ "${total_warnings:-0}" -gt 3 ] && echo "warning" || echo "success")" role="group" aria-labelledby="label-total-warnings">
                <div class="metric-value">${total_warnings}</div>
                <div class="metric-label" id="label-total-warnings">Total Warnings</div>
            </div>
EOF
	fi
