...
4. **Duplicate & overlap:** Detect exact duplicates (>90% file overlap),
   ...
   `tasks/pr-triage.md`. Keep the most recent or most relevant PR per group;
...
- **Post-merge conflict cascade:** Re-check mergeable state after each merge
  before proceeding.
...
- **File-path overlap:** Same files do not alone mean duplicate; confirm
  title/intent before closing as duplicate.
