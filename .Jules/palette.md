...
## 2026-11-20 - Communicating Wait States with Spinners
**Learning:** Arbitrary `sleep` commands in scripts leave users wondering if the script has frozen. Replacing raw sleeps with visual spinners and contextual messages significantly reduces perceived latency and improves the interactive CLI experience.
**Action:** Replace `sleep X` with a `spinner_wait X "Doing something..."` function that provides visual feedback in interactive TTY environments and gracefully falls back to a logging statement + sleep in non-TTY environments.
