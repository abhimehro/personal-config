...
2. **Check Currently Installed Skills:** Determine which skills are already
...
3. **Add Missing or Unlinked Skills:** Compare the results from Step 1 and
   Step 2. For each missing or unlinked skill, follow the appropriate
   conditional instruction below:
...
4. **Update Existing Skills:** Update all currently installed skills to their
   latest versions:
   ```bash
