...
- **Handling**: Ensure Node.js is installed and the version is `>= 20`. If
  ...
  **For macOS or Linux:**
  ...
  2. Request the user to manually install `nvm` and reply when finished. **Stop
     and wait** for the user's confirmation.
  3. Make `nvm` available in the current terminal session by sourcing the
     appropriate profile:
     ```bash
     # For Bash
     source ~/.bash_profile
  ...
  **For Windows:**
  ...
  2. Request the user to manually install `nvm-windows` and Node.js, and reply
  ...
  **Alternative: Official Installer**
  ...
  2. Request the user to manually install Node.js and reply when finished.
