...
2. **Check Global skills**: If not found locally, check the global installation
   by running:
   ```bash
   ...
   globally.
3. **Install Skills**: If both checks fail, run the following command to install
...
4. **Verify Installation**: Re-run the checks in steps 1 or 2 to confirm that
...
2. **Check Existing Configuration**: Open `mcp.json` and check the `mcpServers`
   ...
   **Example valid configurations**:
   ...
   ```
3. **Add or Update Configuration**: If the `firebase` block is missing or
...
4. **Verify Configuration**: Save the file and confirm the `firebase` block is
...
2. **Confirm Connection**: Check the MCP server list in the Cursor UI to confirm
