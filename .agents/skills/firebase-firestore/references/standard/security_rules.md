...
- **The "Ownership-Only Update" Trap:** A common critical vulnerability is
  ...
  `allow update: if
    isOwner(resource.data.uid);`). This allows the owner to
  ...
  `allow update:
    if isOwner(...) && isValidEntity(...);`) **AND** validate
...
3. **The "Update Bypass":** Can I `create` a valid document and then `update` it
