...
2. The `genkit start` command to run in their terminal (foreground — it's
   expected to block)
...
> Get a free key at https://aistudio.google.com/apikey — click **"Create API
> key"**, copy it.
...
Replace `/path/to/your-project` with the actual full path to the project (e.g.
`/Users/yourname/projects/my-genkit-app`):
...
(Requires a project with `pyproject.toml` — run `uv init` in an empty directory
first if needed.)
...
Run this in your terminal. **It will block — that's expected.** Leave this
terminal open while you use the Dev UI.
...
6. Click **"Traces"** in the left sidebar to inspect every step, model call,
