...
**Control D note (2026-07-09):** Live host confirmed **WORKING / local_fallback** (brew ctrld v1.5.3). Prefer `./scripts/controld-status.sh`; repair only if BROKEN: `sudo ./scripts/repair-controld-keepalive.sh --restart privacy`. CD Mode argv must **not** include `--listen` (Lesson 0do). Colima coexistence uses Lima `override.yaml`, not only `colima.yaml`.
