...
update_progress_if_needed() {
	...
	# SECURITY: Prevent command injection by validating variable name
	...
	# Get last update time from variable safely using indirect expansion
	local last_time
	...
	if [[ $((current_time - last_time)) -ge $interval ]]; then
		...
		# Update the last_update_time variable safely using printf
