...
#
# Dependencies:
#   - debug_log (from lib/core/log.sh)
#   - get_path_size_kb (from lib/core/file_ops.sh)
#   - run_with_timeout (from lib/core/timeout.sh)
...
brew_uninstall_cask() {
	...
	local cask_gone=false app_gone=true
	is_brew_cask_installed "$cask_name"
	local cask_state=$?
	# Exit 1 = not installed (success), exit 2 = unknown (treat as failure for safety)
	[[ $cask_state -eq 1 ]] && cask_gone=true
