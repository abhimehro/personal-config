...
def run_weekly_retrospective(config: dict[str, Any]) -> dict[str, Any]:
    ...
    # ⚡ Bolt Optimization: Parallelize independent API calls to reduce execution latency
    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
        future_runs = executor.submit(recent_daily_runs)
        future_markers = executor.submit(
            weekly_markers,
            config.get("reporting", {}).get(
                "daily_issue_prefix", "[repo-automation] Daily Status Report"
            ),
        )
        runs = future_runs.result()
        markers = future_markers.result()
