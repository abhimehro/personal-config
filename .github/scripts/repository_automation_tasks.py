...
def _process_future_result(bucket_name: str, item: dict[str, Any], future: concurrent.futures.Future) -> dict[str, Any]:
...
def execute_configured_commands(
    ...
    # Partition into setup (must run sequentially first, in order) and
    # parallel buckets (commands + security). Setup commands often install
    # dependencies that subsequent commands rely on, so they cannot be
    # parallelized with the rest.
    all_items = list(configured_commands(section))
    setup_items = [(b, i) for b, i in all_items if b == "setup"]
    parallel_items = [(b, i) for b, i in all_items if b != "setup"]

    # Run setup commands sequentially to preserve ordering guarantees.
    ...
    # Validation and security commands have no inter-dependencies, so they
...
def close_invalid_prs(branch_prefix: str) -> None:
    """Close PRs with invalid action version tags."""
    open_prs = gh_json(
    ...
    for pr in open_prs:
        ...
        if _pr_has_invalid_tags(pr.get("body", "")):
            print(
                f"Closing PR #{pr['number']} because it contains invalid tag updates."
            )
            proc = run_process(
                [
                    GH_BIN,
                    ...
                    str(pr["number"]),
                    "--comment",
                    "Closing this draft PR because it contains one or more invalid action versions. A corrected PR will be opened automatically.",
                ]
            )
            if proc.returncode != 0:
                stderr_msg = (
                    proc.stderr.strip()
                    if proc.stderr
                    else proc.stdout.strip() if proc.stdout else "Unknown error"
                )
                print(
                    f"Warning: Failed to close PR #{pr['number']}: {stderr_msg}",
                    file=sys.stderr,
                )
