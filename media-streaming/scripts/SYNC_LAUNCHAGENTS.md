...
Stow-style deployer for the LaunchAgent plists under this repo. Replaces the previous symlink approach with **real file copies** in `~/Library/LaunchAgents/`, eliminating Mole's "orphan/stale login item" false positives and `launchctl bootstrap` EIO 5 errors caused by dangling symlinks.
...
3. If different (or destination is a symlink/missing):
   ...
   * `pkill -f` any lingering process whose `ProgramArguments` script path is still alive (prevents EIO 5).
   * `cp` from source to destination (atomic, mode 644).
   * `launchctl bootstrap gui/$UID <plist>`, falls back to `launchctl load -w` if bootstrap fails.
   * `launchctl enable gui/$UID/<label>`.
4. Writes a TSV manifest at `~/.local/state/sync-launchagents/manifest.tsv` for diff tracking.

## Why not symlinks?

* Mole's `clean_orphaned_app_data` and `show_user_launch_agent_hint_notice` flag broken symlinks as "stale login items" and remove them.
* `launchctl bootstrap` occasionally errors with `5: Input/output error` when handed a freshly re-pointed symlink.
* Real file copies survive `mo purge`, Time Machine restores, and most macOS upgrades.
* The git-tracked plists in this repo remain the canonical source. Edits flow `repo → sync-launchagents → ~/Library/LaunchAgents/`.
...
Run `mo clean --dry-run` after large repo refactors to verify nothing in this set appears as a candidate.
...
If you want sync-on-login (e.g. after a Mole sweep), add a LaunchAgent that calls this script with `RunAtLoad`. Keep the agent itself in the repo so it sync's itself, the chicken-and-egg is fine because launchd will just reload it next login.
