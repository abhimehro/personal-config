...
for d in "${SRC_DIRS[@]}"; do
	[[ -d $d ]] || {
		dbg "skipping missing source dir: $d"
		continue
	}
...
unload_label() {
	...
	if is_loaded "$label"; then
		...
		local i=0
		while is_loaded "$label" && [[ $i -lt 10 ]]; do
			sleep 0.2
			i=$((i + 1))
		done
	fi
...
for src in "${SRC_PLISTS[@]}"; do
	...
	if [[ $FORCE == "true" ]]; then
		...
		needs_install=true
		action="convert-symlink-to-file"
...
if [[ $PRUNE == "true" ]]; then
	...
	while IFS= read -r -d '' dst; do
		...
		if "$is_broken_symlink" || [[ $label == ${LABEL_PREFIX_FILTER}* ]]; then
			...
			if [[ $DRY_RUN == "true" ]]; then
				dry
				echo "would unload + remove $fname"
				continue
			fi
			unload_label "$label"
			rm -f "$dst"
