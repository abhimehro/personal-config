...
class MediaServerHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        try:
            if path == "":
                ...
                result = subprocess.run(
                    ["rclone", "lsf", f"{self.rclone_remote}{path}/"],
                    capture_output=True,
                    text=True,
                )
                if result.returncode == 0:
    ...
    def stream_file(self, path):
        ...
        try:
            ...
            process = subprocess.Popen(
                ["rclone", "cat", f"{self.rclone_remote}{path}"],
    ...
    def generate_directory_listing(self, files, current_path):
        ...
        if not items_html:
            items_html.append(
                '<li class="empty-state"><span class="file" role="status" style="color: #666;"><em>Directory is empty</em></span></li>\n'
            )
...
def main():
    ...
    if args.public:
        print("⚠️  Public Access Enabled (0.0.0.0). Ensure your network is trusted.")

    try:
        # ⚡ Performance: Use ThreadingTCPServer to handle concurrent requests (e.g. streaming + browsing)
        with socketserver.ThreadingTCPServer(
            (host, args.port), MediaServerHandler
        ) as httpd:
