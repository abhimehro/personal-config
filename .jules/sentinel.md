...
## 2026-10-15 - Fix Command Injection (CWE-78) in base.sh get_user_home
**Vulnerability:** A Command Injection ([CWE-78](https://cwe.mitre.org/data/definitions/78.html)) vulnerability was found in `configs/.config/mole/lib/core/base.sh`. The `get_user_home` function directly passed the `user` string into an `eval echo "~$user"` command without prior input validation, allowing arbitrary OS command execution if untrusted or dynamic variables were provided.
**Learning:** Even internal helper scripts must strictly validate input when performing dynamic expansion using `eval`. Without strict bounds (like a regex allowlist), arbitrary commands can be injected via the variable value (e.g., `user="root; touch /tmp/pwned"`). Using safer alternatives like `getent` provides secure fallback behavior before relying on `eval`.
**Prevention:** Strictly validate dynamic values using a rigid regex allowlist (e.g., `^[a-zA-Z0-9_][a-zA-Z0-9_.-]*\$?$`) before any `eval` execution. Always prioritize secure system tools like `dscl` or `getent` for looking up data over command evaluation.
