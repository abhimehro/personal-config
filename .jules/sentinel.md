...
## 2026-05-26 - Insecure Temporary Directory in Surgical Cleanup

**Vulnerability:** An Insecure Temporary File Creation ([CWE-377](https://cwe.mitre.org/data/definitions/377.html)) vulnerability was found in `scripts/macos/SURGICAL_CLEANUP.sh`. The script used a predictable, hardcoded temporary path (`/tmp/fish_backup`) to temporarily store the user's `fish` configuration during cleanup.
**Learning:** Using a predictable path in a world-writable directory like `/tmp` allows a local attacker to pre-create the directory as a symbolic link. When the script attempts to move the backed-up files back to their original location (`mv "/tmp/fish_backup" "$HOME/.config/fish"`), it could follow the symlink, leading to arbitrary file overwrite or backup hijacking with the script runner's privileges.
**Prevention:** Always generate unique, unpredictable temporary directories using `mktemp -d -t 'prefix.XXXXXX'` for storing backups during multi-step file operations. Ensure the directory is securely removed afterward.

## 2025-01-31 - Command Injection Risk via eval in Home Directory Resolution
...
## 2026-11-20 - Insecure Temporary File Creation (CWE-377) in LaunchAgent
...
## 2026-06-21 - Option Injection Risk via pgrep/pkill
...
## 2026-03-23 - Command Injection Risk via subprocess.run with shell=True

**Vulnerability:** Command Injection ([CWE-78](https://cwe.mitre.org/data/definitions/78.html)) risk existed in `parse_inventory.py`. The code constructed a GitHub CLI command string dynamically using `f"source ../email-security-pipeline/GH_TOKEN.env && gh pr view {pr} -R {repo} --json ..."` and executed it using `subprocess.run(..., shell=True)`. Because `pr` and `repo` were parsed from a markdown file (`tasks/pr-inventory.md`), an attacker could potentially control these values to inject arbitrary shell commands.
**Learning:** Using `subprocess.run` with `shell=True` and dynamically interpolated strings is a classic command injection vulnerability, effectively equivalent to using `eval` or `system()` in other languages. While `source` requires a shell to run, relying on the shell to parse `.env` files is dangerous when mixed with untrusted input in the same command.
...
## 2026-04-17 - Command Injection via shell=True in Multiple Automation Scripts

**Vulnerability:** Command Injection ([CWE-78](https://cwe.mitre.org/data/definitions/78.html)) risk in `categorize_ready.py`, `detect_duplicates.py`, and `run_merges.py`. These scripts used `subprocess.run(f"... && gh pr view {pr_id} ...", shell=True)` where `pr_id` and `repo` were interpolated into a shell string.
**Learning:** Reusing a vulnerable pattern across multiple scripts compounds risk. Even if variables currently appear "safe" (e.g. from hardcoded lists), they often originate from external files (like `tasks/pr-triage.md`) in other scripts, violating trust boundaries. Relying on `source` via `shell=True` for environment loading is a common but dangerous shortcut.
**Prevention:** Standardize on a secure `run_gh` utility that uses `shell=False` and a list of arguments. Implement a manual `.env` file parser to populate the `env` parameter of `subprocess.run` instead of using the shell's `source` built-in.
