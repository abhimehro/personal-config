...
## 2026-03-24 - Command Injection Risk via subprocess.run with shell=True in script logic

**Vulnerability:** Command Injection ([CWE-78](https://cwe.mitre.org/data/definitions/78.html)) risk existed in `run_merges.py`, `detect_duplicates.py`, and `categorize_ready.py`. The code constructed GitHub CLI command strings dynamically using interpolation and executed them using `subprocess.run(..., shell=True)`. This could allow command injection if the interpolated strings contained shell metacharacters.
**Learning:** Using `subprocess.run` with `shell=True` and dynamically interpolated strings is a classic command injection vulnerability. While `source` requires a shell to run, relying on the shell to parse `.env` files is dangerous when mixed with untrusted input in the same command.
**Prevention:** Avoid `shell=True` in `subprocess.run`. Pass commands and arguments as a list of strings. If you need to load environment variables, parse the `.env` file manually in Python and pass the resulting dictionary to the `env` parameter of `subprocess.run`.
