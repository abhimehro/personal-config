...
## 2024-04-12 - Prevent Command Injection in subprocess calls
**Vulnerability:** Several Python scripts in the root directory (parse_inventory.py, detect_duplicates.py, run_merges.py, categorize_ready.py) were using `subprocess.run(cmd, shell=True)` with string concatenation. `run_merges.py` was also using shell-specific built-ins like `source` with string concatenation for parameters, which allowed for command injection via shell metacharacters in pull request or repository variables.
**Learning:** `shell=True` exposes the application to command injection if any part of the command string is derived from untrusted input (like repository names or PR numbers from API responses or files).
**Prevention:** Always use `shell=False` (which is the default) with `subprocess.run` by passing arguments as a list of strings. When environment variables from a file are needed, manually parse the `.env` file and pass them using the `env` argument instead of relying on the bash `source` command.
