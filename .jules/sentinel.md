...
## 2026-10-27 - Command Injection Risk via eval in Timestamp Assignment

**Vulnerability:** Command Injection ([CWE-78](https://cwe.mitre.org/data/definitions/78.html)) risk existed in `maintenance/bin/system_cleanup.sh`. The `get_timestamp` function (used when bash version is older than 4) constructed variables dynamically by assigning a date string and evaluating it using `eval "$1='$val'"`.
**Learning:** Using `eval` for dynamic variable assignment is an insecure anti-pattern. While `$val` is known (`date` output), `$1` is passed dynamically. If `$1` is ever influenced by unsanitized input, it can lead to arbitrary code execution.
**Prevention:** Avoid `eval` for dynamic variable assignment. Use `printf -v "$1" "%s" "$val"` instead, which securely assigns the string value directly to the variable named by `$1` without passing through the shell execution engine.
