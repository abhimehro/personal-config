...
**Prevention:** Remove `eval` entirely and avoid Bash-isms like `printf -v`. Prefer POSIX-compliant standard command substitution (e.g., `ts="$(get_timestamp)"`) for variable assignment, as CI environments often execute scripts via `/bin/sh` (dash).
