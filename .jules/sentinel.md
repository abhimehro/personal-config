...
## 2026-04-11 - Command Injection Risk via shell=True and untrusted config sourcing
**Vulnerability:** A Command Injection ([CWE-78](https://cwe.mitre.org/data/definitions/78.html)) vulnerability existed in `parse_inventory.py`. The `run_gh` function used `subprocess.run(f"source ../email-security-pipeline/GH_TOKEN.env && gh pr view {pr} ...", shell=True)`. The variables `{pr}` and `{repo}` came from external PR bodies (parsed from `tasks/pr-inventory.md`), which is untrusted user input that could contain shell metacharacters. Additionally, `source` executes the file in the current shell, making it a risk if the `.env` file is manipulated.
**Learning:** Using `shell=True` to enable sourcing `.env` files exposes the command to injection if any dynamically interpolated string is derived from untrusted input. Sourcing a file is also equivalent to executing it, making it inherently risky.
**Prevention:** Avoid `shell=True` and instead construct a list of arguments for `subprocess.run`. If environment variables from a file are needed, manually parse the file in code (e.g., stripping `export ` and quotes) and pass the resulting dictionary using the `env=` parameter of `subprocess.run()`.
