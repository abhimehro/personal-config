...
## 2026-03-24 - Option Injection Risk via pgrep/pkill with unseparated variables
**Vulnerability:** Option Injection (CWE-88) risk existed in `configs/.config/mole/lib/clean/system.sh` and `configs/.config/mole/lib/core/app_protection.sh`. Functions used `pgrep -x "$match_pattern"` and `pgrep -f "$installer_app"`. If an attacker could control the variable passed to these commands (or if an app was intentionally named starting with a hyphen, e.g., `-u`), `pgrep` and `pkill` would interpret the value as a command-line option rather than the target pattern, leading to unexpected behavior or potential exploitation.
**Learning:** Shell commands that accept variable input for search patterns or filenames are vulnerable to option injection if the variable starts with a hyphen.
**Prevention:** Always use the `--` end-of-options separator (e.g., `pgrep -x -- "$pattern"`) to explicitly signal to the utility that subsequent arguments should be treated as operands, even if they begin with a hyphen.
