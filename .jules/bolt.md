...
## 2025-10-27 - Bash Built-ins vs Pipelines
**Learning:** In frequently executed shell functions (like status checks or loops), replacing pipelines (e.g., `echo | tr | sed`) with Bash built-in parameter expansion (e.g., `${var//pattern/repl}`) significantly reduces overhead by avoiding process creation.
**Action:** Always prefer Bash built-ins for simple string manipulation over calling external tools like `grep`, `sed`, or `tr`.
