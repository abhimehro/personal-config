...
| What                       | Command                                          | Notes                                                                                                                |
| -------------------------- | ------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------- |
| Cursor Cloud hook sync     | `make cursor-cloud-hooks`                        | Copies `scripts/cursor_cloud_agent_*.sh` into `~/.cursor/agent-hooks/*` (printenv fix for spaced secret names)        |
...
**Fresh Cloud workspaces** may ship older injected copies. After clone, run **`make cursor-cloud-hooks`** (or `./scripts/install_cursor_cloud_agent_hooks.sh`) once per session to overwrite the injected hooks with the canonical scripts from this repo. To target one hash directory: `CURSOR_AGENT_HOOKS_DIR=~/.cursor/agent-hooks/<hash> ./scripts/install_cursor_cloud_agent_hooks.sh`.
