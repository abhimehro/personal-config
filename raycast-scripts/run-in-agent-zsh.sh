...
# @raycast.title Run in Agent Zsh
...
# @raycast.description Run a command inside agent-zsh -c (avoids Fish)
...
CMD=""
if [[ -z "" ]]; then
  echo "Provide a command argument" >&2
  exit 2
fi
if [[ -x "/Users/speedybee/bin/agent-zsh" ]]; then
  exec "/Users/speedybee/bin/agent-zsh" -c ""
fi
