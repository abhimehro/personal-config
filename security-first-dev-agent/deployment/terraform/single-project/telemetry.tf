...
resource "google_bigquery_dataset" "telemetry_dataset" {
  # checkov:skip=CKV_GCP_81:Google-managed encryption keys are sufficient for dev telemetry dataset
...
resource "google_bigquery_table" "completions_external_table" {
  # checkov:skip=CKV_GCP_80:Google-managed encryption keys are sufficient for completions external table
  # checkov:skip=CKV_GCP_121:Deletion protection is not needed for external tables referencing GCS
  project             = var.project_id
  dataset_id          = google_bigquery_dataset.telemetry_dataset.dataset_id
...
resource "google_bigquery_table" "genai_logs_table" {
  # checkov:skip=CKV_GCP_80:Google-managed encryption keys are sufficient for log export table
  # checkov:skip=CKV_GCP_121:Deletion protection is not required for log export table
  project             = var.project_id
  dataset_id          = google_bigquery_dataset.telemetry_dataset.dataset_id
...
resource "google_bigquery_table" "completions_view" {
  # checkov:skip=CKV_GCP_80:Google-managed encryption keys are sufficient for completions view
  # checkov:skip=CKV_GCP_121:Deletion protection is not required for logical views
  project             = var.project_id
  dataset_id          = google_bigquery_dataset.telemetry_dataset.dataset_id
