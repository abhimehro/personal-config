...
## Lesson 0eg: Overlapping session-artifact PRs — merge one, close the other (2026-05-22)

**Pattern:** `personal-config#1011` and `#1013` both updated `tasks/pr-inventory.md`, `tasks/pr-triage.md`, and `tasks/lessons.md`. Merging #1011 left #1013 `CONFLICTING`; `update-branch` returned 422.
**Rule:** For Cursor/app session doc PRs on the same date, merge the **review** artifact first, then **close** the salvage artifact as superseded if it only duplicates task docs. Do not block the run waiting for manual conflict resolution on meta-docs.
