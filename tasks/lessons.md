...
also touches CI trust-boundary surfaces (Gemini agents, secret scanners). Leave
ESCALATE with an explicit keep-vs-delete note for the maintainer. **Detection
cost:** Low — `git merge-tree` / `git merge --no-commit` shows
