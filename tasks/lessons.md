...
4. Strengthen the test with at least one defense-in-depth assertion (e.g. for #826, the original asserted only "no `shell=True`"; the salvage added "`_load_gh_token_env` must not call `subprocess.{run,Popen}`" so a future shell-out via env-loader is also caught).
...
## Lesson 0dt: Workflow automation must bump SHAs forward, never replace SHAs with tags (2026-05-30)

**Pattern:** ESP #962 (`automation-workflow-updates-20260530-1`) replaced full commit SHAs with version tags (`actions/github-script@v9.0.0`, `upload-sarif@codeql-bundle-v2.25.5`). The `bandit` job failed before scan: org policy requires full-length SHAs on every action reference.
**Rule:** Close automation PRs that move **from SHA → tag** without opening a salvage branch. Valid workflow bumps must resolve each tag to a **new** full SHA and keep `main` green. Read the failed job log — resolution errors often appear before application tests run.
**Detection cost:** Low — `gh pr diff` shows `@v` or `codeql-bundle-*` on `uses:` lines; confirm with `gh run view --log-failed`.
