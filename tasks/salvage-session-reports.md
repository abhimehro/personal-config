...
| --------------- | -----: | --------------------------- | --------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| personal-config |   2041 | SALVAGE (draft; original left ready) | [#2063](https://github.com/abhimehro/personal-config/pull/2063) | Markers + stale skip-row; `automation` title + `cursor-agent/` branch; `make lint-errors` PASS |
...
- pc #2063: `make lint-errors`; re-read `isDraft=true`
- ctrld #1161: `display.py` 404 on main; generator-form lives in `display/tables.py`

### Handoff

1. Stage 1: re-ingest draft **#2063** for `/trunk merge` if routine predicates pass
