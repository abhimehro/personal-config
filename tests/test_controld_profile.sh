...
test_validate_profile_id() {
    if ! validate_profile_id "valid-ID_123"; then
        echo "Fail: valid-ID_123 should be valid"
        return 1
    fi
    if validate_profile_id "invalid ID!"; then
        echo "Fail: 'invalid ID!' should be invalid"
        return 1
    fi
    if validate_profile_id ""; then
        echo "Fail: empty string should be invalid"
        return 1
    fi
    return 0
}
...
if ! test_validate_profile_id; then echo "test_validate_profile_id failed"; test_failed=1; fi
