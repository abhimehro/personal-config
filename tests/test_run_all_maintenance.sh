...
if bash "$MOCK_DIR/run_all_maintenance.sh" help > "$TEST_DIR/t1.log" 2>&1; then
    echo "PASS: help exits 0"; PASS=$((PASS + 1))
else
    echo "FAIL: help exited non-zero"; cat "$TEST_DIR/t1.log"; FAIL=$((FAIL + 1))
fi
...
if [[ "$t2_exit" -ne 0 ]]; then
    ...
    echo "FAIL: unknown mode should exit non-zero"; FAIL=$((FAIL + 1))
fi

# ---- Test 3: health mode dispatches to health_check.sh ----
> "$CALL_LOG"
if bash "$MOCK_DIR/run_all_maintenance.sh" health > "$TEST_DIR/t3.log" 2>&1; then
    echo "PASS: health mode exits 0"; PASS=$((PASS + 1))
else
    echo "FAIL: health mode exited non-zero"; cat "$TEST_DIR/t3.log"; FAIL=$((FAIL + 1))
fi
...
if bash "$MOCK_DIR/run_all_maintenance.sh" quick > "$TEST_DIR/t4.log" 2>&1; then
    echo "PASS: quick mode exits 0"; PASS=$((PASS + 1))
else
    echo "FAIL: quick mode exited non-zero"; cat "$TEST_DIR/t4.log"; FAIL=$((FAIL + 1))
fi
...
if bash "$MOCK_DIR/run_all_maintenance.sh" weekly > "$TEST_DIR/t5.log" 2>&1; then
    echo "PASS: weekly mode exits 0"; PASS=$((PASS + 1))
else
    echo "FAIL: weekly mode exited non-zero"; cat "$TEST_DIR/t5.log"; FAIL=$((FAIL + 1))
fi
...
if [[ "$log_count" -gt 0 ]]; then
    ...
    echo "FAIL: master log file not found under $LOG_TMP"; FAIL=$((FAIL + 1))
fi
...
if echo "$t8_out" | grep -q "already running"; then
    echo "PASS: concurrent lock blocks second instance"; PASS=$((PASS + 1))
else
    echo "FAIL: expected 'already running' message; got: $t8_out"; FAIL=$((FAIL + 1))
fi
if ! grep -q "health_check.sh called" "$CALL_LOG" 2>/dev/null; then
    echo "PASS: no sub-scripts called while locked"; PASS=$((PASS + 1))
else
    echo "FAIL: sub-scripts were invoked despite active lock"; FAIL=$((FAIL + 1))
fi
...
if [[ "$call_count" -eq 2 ]]; then
    ...
    echo "FAIL: expected 2 health_check.sh dispatches, got $call_count"; FAIL=$((FAIL + 1))
fi
