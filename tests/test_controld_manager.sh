...
generate_profile_config() {
	echo "mock_generate_profile_config"
	return 0
}
restart_with_config() {
	echo "mock_restart_with_config"
	return 0
}
...
emergency_recovery() {
	echo "mock_emergency_recovery"
	return 0
}
show_status() {
	echo "mock_show_status"
	return 0
}
pgrep() { return 1; }
...
# ── Run all tests ─────────────────────────────────────────────────────────────
if ! test_switch_profile_valid; then
	echo "test_switch_profile_valid failed"
	test_failed=1
fi
if ! test_switch_profile_invalid; then
	echo "test_switch_profile_invalid failed"
	test_failed=1
fi
if ! test_switch_profile_doh_protocol; then
	echo "test_switch_profile_doh_protocol failed"
	test_failed=1
fi
...
if ! test_main_commands; then
	echo "test_main_commands failed"
	test_failed=1
fi
if ! test_switch_missing_profile; then
	echo "test_switch_missing_profile failed"
	test_failed=1
fi
if ! test_init_command; then
	echo "test_init_command failed"
	test_failed=1
fi
if ! test_emergency_command; then
	echo "test_emergency_command failed"
	test_failed=1
fi
if ! test_test_command; then
	echo "test_test_command failed"
	test_failed=1
fi
if ! test_unknown_command; then
	echo "test_unknown_command failed"
	test_failed=1
fi
if ! test_switch_profile_restores_on_failure; then
	echo "test_switch_profile_restores_on_failure failed"
	test_failed=1
fi
# $MOCK_BIN subprocess smoke tests
if ! test_smoke_status; then
	echo "test_smoke_status failed"
	test_failed=1
fi
if ! test_smoke_help; then
	echo "test_smoke_help failed"
	test_failed=1
fi
if ! test_smoke_stop_requires_root; then
	echo "test_smoke_stop_requires_root failed"
	test_failed=1
fi
