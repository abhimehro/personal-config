...
if [[ -f "$MOCK_HOME/.config/media-server/credentials" ]]; then
	echo "PASS: credentials file created on first run"
	PASS=$((PASS + 1))
else
...
if [[ $GENERATED_USER == "infuse" ]]; then
	echo "PASS: generated credentials use default user 'infuse'"
	PASS=$((PASS + 1))
else
	echo "FAIL: generated credentials user is '$GENERATED_USER', expected 'infuse'"
	FAIL=$((FAIL + 1))
fi

if [[ -n $GENERATED_PASS ]]; then
	echo "PASS: generated credentials password is non-empty"
	PASS=$((PASS + 1))
else
	echo "FAIL: generated credentials password is empty"
	FAIL=$((FAIL + 1))
fi
...
if [[ -n $GENERATED_PASS ]]; then
	...
	echo "FAIL: skipping rclone password pattern check because generated password is empty"
	FAIL=$((FAIL + 1))
fi
...
if [[ $MOCK_PASS == "test-media-server-password" ]]; then
	echo "PASS: security mock returns expected password"
	PASS=$((PASS + 1))
else
	echo "FAIL: security mock returned unexpected output: $MOCK_PASS"
	FAIL=$((FAIL + 1))
fi
...
if [[ $MOCK_EXIT -ne 0 ]]; then
	echo "PASS: security mock exits non-zero for unsupported operations"
	PASS=$((PASS + 1))
else
	echo "FAIL: security mock should exit non-zero for unsupported operations"
	FAIL=$((FAIL + 1))
fi
