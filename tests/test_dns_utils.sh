...
# Run tests
if ! test_backup_network_settings; then
	echo "test_backup_network_settings failed"
	test_failed=1
fi
if ! test_restore_network_settings; then
	echo "test_restore_network_settings failed"
	test_failed=1
fi
if ! test_test_dns_resolution; then
	echo "test_test_dns_resolution failed"
	test_failed=1
fi
