...
from adguard.scripts.extract_domains import extract_allowlist_domains_from_file


class TestExtractAllowlistDomainsFromFile(unittest.TestCase):

    def test_happy_path(self):
        """Happy path: valid JSON, rules key exists, extracts only PK values where action.do == 1."""
        json_data = json.dumps(
            {
                "rules": [
                    {"PK": "example.com", "action": {"do": 1}},
                    {"PK": "bad.com", "action": {"do": 0}},
                    {"PK": "test.com", "action": {"do": 1}},
                    {"PK": "missing-do.com", "action": {}},
                ]
            }
        )
        with tempfile.NamedTemporaryFile(
    ...
    def test_partial_data(self):
        """Partial data: missing PK, missing action, or missing do, skips cleanly."""
        json_data = json.dumps(
            {
                "rules": [
                    {"PK": "valid.com", "action": {"do": 1}},
                    {"action": {"do": 1}},  # missing PK
                    {"PK": "no-action.com"},  # missing action
                    {"PK": "also-valid.com", "action": {"do": 1}},
                ]
            }
        )
        with tempfile.NamedTemporaryFile(
    ...
    def test_missing_rules(self):
        """Missing rules: valid JSON without rules, returns empty list."""
        json_data = json.dumps(
            {"different_key": [{"PK": "example.com", "action": {"do": 1}}]}
        )
        with tempfile.NamedTemporaryFile(
            mode="w", delete=False, encoding="utf-8"
        ) as tf:
    ...
    def test_missing_file(self, mock_print):
        """Missing file: prints an error, returns empty list."""
        missing_path = "nonexistent_file_allowlist_12345.json"
