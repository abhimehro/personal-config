...
def process_allowlist_files(base_dir):
    ...
    allowlist_domains.update(
        extract_allowlist_from_file(tlds_file, "legitimate TLD domains")
    )
...
def create_json_structure(domains, group_name, action_do):
    ...
    return {
        "group": {"group": group_name, "action": {"do": action_do, "status": 1}},
        "rules": [
            {"PK": domain, "action": {"do": action_do, "status": 1}}
...
def write_json_files(output_dir, denylist_domains, allowlist_domains):
    ...
    denylist_json = create_json_structure(
        denylist_domains, "Comprehensive Tracker Denylist", 0
    )
    allowlist_json = create_json_structure(
        allowlist_domains, "Comprehensive Allowlist", 1
    )
