...
def write_text_files(base_dir, sorted_denylist, sorted_allowlist):
    ...
    with open(denylist_txt_path, "w", encoding="utf-8") as f:
        ...
        f.write(
            "# This file contains all domains from various tracker lists that should be blocked\n"
        )
        f.write(
...
def write_json_files(base_dir, sorted_denylist, sorted_allowlist):
    ...
    denylist_json = {
        ...
        "rules": [
            {"PK": domain, "action": {"do": 0, "status": 1}}
    ...
    allowlist_json = {
        "group": {"group": "Comprehensive Allowlist", "action": {"do": 1, "status": 1}},
        "rules": [
            {"PK": domain, "action": {"do": 1, "status": 1}}
