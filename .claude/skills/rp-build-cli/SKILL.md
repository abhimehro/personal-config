...
The quick scan is orientation only — `builder` does the deep exploration and
produces the plan. Skipping it tends to produce shallow implementations that
...
Implement the plan directly. Don't use `chat` with `mode:"edit"` — you implement
directly.
...
- 🚫 Extended reading before calling `builder` – a quick skim is fine; let the
  builder do the heavy lifting
