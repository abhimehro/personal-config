...
**not** create each salvage branch independently off `main`. Instead, build them
as a chained stack with
`gh stack init salvage/<repo>-<a> salvage/<repo>-<b> ...`, then
`gh stack rebase --no-trunk` and `gh stack submit --auto` to open them all as
**draft** PRs. Each layer inherits the layer below it, so a salvage that depends
on an infra-fix or a lower-priority salvage is already rebased on it —
eliminating the domino conflicts that normally follow when siblings are merged
one-by-one. This does not violate S1 (never merge autonomously): stacking only
changes how the draft PRs are _created_, not how they are merged. Merging still
requires a human, and still uses the merge-async path documented in AGENTS.md
(`gh stack merge --yes` or `PUT /repos/{owner}/{repo}/pulls/{n}/merge-async`),
never `gh pr merge` or auto-merge.
