...
6. **Option 3 handoff:** Mechanical CONFLICTING `HOLD_CONTRACT` / wrap in STAGE3_RECONCILIATION → `HANDOFF_MECHANICAL_TO_STAGE2` (complete Stage 2 WI, owner stage2). Do **not** spend the reconciliation cap on CLOSED_NOOP Observed-CLOSED when Stage 1 bookkeeping / weekly can take them (`CLOSED_NOOP_DEFERRED`).
