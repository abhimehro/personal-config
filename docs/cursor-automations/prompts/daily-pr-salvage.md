...
4. **Skip-if-empty (Option 3):** If dry-run `pipeline_health.stage2_work_item_count == 0` **and** post never-touch mechanical candidates == 0 (plan reason `EMPTY_INTAKE_SKIP` / action `SKIP_IF_EMPTY`) → **exit success**; do not open/push a docs PR; do not launch further agents. Optional one-line status only.
