...
continue. That reset is not a successful calibration run. If preflight fails on
this policy error, read the ledger's Contents metadata and fetch that exact blob;
reset and validate those bytes, commit with the metadata's `sha` as
`--base-blob-sha`, then run preflight again. Do not use a SHA from an earlier
preflight. Contents GET of the runtime ledger returns `encoding: none` above 1 MB; fetch bytes with
...
names `gh` (read-only), Notion for one-question packets, Linear only if packets
are filed there, and codescene/Snyk/Sonatype-mcp as hold evidence rather than
merge gates. GitKraken is optional and only if actually up. GitBook, GitHits,
and Confidence-docs are out of scope unless a PR actually touches that product.
