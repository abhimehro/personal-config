...
waiting for the next stage). Stopping is honest only when empty
intake has zero salvage-eligible remainder, or a true
`HOLD_PLATFORM` / `ANALYSIS_ERROR` blocks every mutation. If an
earlier stage did incomplete or incorrect work, repair it in this
run and continue. Guardrails still bind: never merge drafts, never
self-approve under maintainer login.
...
- `REVIEW.md`: rank findings by consequence (Blocking / Discuss /
  ...
  comment per root cause; do not flood optional nits.
