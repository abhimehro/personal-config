...
out_md = []
out_md.append(f"# Automated PR inventory — backlog cleanup test ({datetime.date.today().isoformat()})\n")
...
def get_category(title, branch):
    l = (title + branch).lower()
    if "sentinel" in l or "security" in l or "injection" in l or "cwe" in l or "ssrf" in l or "tls" in l: return "SECURITY"
    if "bolt" in l or "perf" in l or "optimize" in l: return "PERFORMANCE"
    if "palette" in l or "ux" in l or "ui" in l: return "UI"
    if "qa" in l or "test" in l or "ci" in l or "infra" in l or "action" in l: return "CI/INFRA"
    if "refactor" in l or "import" in l or "clean" in l: return "REFACTOR"
    return "FEATURE"

for pr in sorted(all_prs, key=lambda x: (x['repo'], -x['number'])):
    cat = get_category(pr['title'], pr['headRefName'])
    ci = "C" if pr['mergeStateStatus'] in ("CLEAN", "HAS_HOOKS") else "U" if pr['mergeStateStatus'] == "UNSTABLE" else "D" if pr['mergeStateStatus'] == "DIRTY" else "?"
    conflicts = "yes" if pr['mergeStateStatus'] == "DIRTY" else "none"
