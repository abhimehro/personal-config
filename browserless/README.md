...
but scrubs filled values. Take any captures **before** the first fill, or in a
separate session.

Recommended pattern: fill credentials once, sign in, then capture the signed-in
state with `Browserless.saveProfile`. Later sessions reuse `?profile=<name>`
with no fill, so captures work normally.
