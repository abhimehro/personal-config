...
*)
	...
	echo ""
	echo -e "${BOLD}When to disable IPv6?${NC}"
	echo -e "  - Windscribe IPv4-only / static IP locations (leak prevention)"
	echo -e "  - Apps attempting IPv6 outside an IPv4-only tunnel will timeout/fail"
	echo -e "  - Force-disable anytime with: sudo $0 disable"
	echo ""
	echo -e "${BOLD}When to enable IPv6?${NC}"
	echo -e "  - Standalone Control D (DoH3) mode"
	echo -e "  - Windscribe WireGuard locations with IPv6 egress (DoH + IPv6)"
	echo ""
	echo -e "${BOLD}Examples:${NC}"
	echo -e "  sudo $0 disable    # Disable IPv6 (IPv4-only VPN / leak prevention)"
	echo -e "  $0 status          # Check current status"
	echo -e "  sudo $0 enable     # Re-enable IPv6 (Control D or IPv6-capable VPN)"
	;;
esac
