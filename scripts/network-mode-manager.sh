...
print_status() {
  ...
  if pgrep -x "ctrld" >/dev/null 2>&1; then
    ...
    local running_uid
    running_uid=$(sudo ctrld status 2>/dev/null | awk '/Resolver ID/ {print $NF}' 2>/dev/null || echo "N/A")
  ...
  if [[ "$dns_servers" == *"There aren't any DNS Servers"* ]]; then
    ...
    local cleaner_dns
    cleaner_dns="${dns_servers//$'\n'/, }"
    # Remove potentially trailing comma+space if dns_servers had trailing newline
