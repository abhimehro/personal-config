...
if [[ $MODE == "doh3-ipv6" ]]; then
	pass "PROFILE_MODE=doh3-ipv6"
else
	fail "expected doh3-ipv6 got '$MODE'"
fi

info "=== Mode 2: doh-ipv4 (DoH + IPv6 off) ==="
./scripts/network-mode-manager.sh controld "$PROFILE" doh || fail "switch controld doh"
MODE=$(sudo awk -F= '$1=="PROFILE_MODE"{print $2;exit}' /etc/controld/active_profile 2>/dev/null || echo "")
if [[ $MODE == "doh-ipv4" ]]; then
	pass "PROFILE_MODE=doh-ipv4"
else
	fail "expected doh-ipv4 got '$MODE'"
fi
IPV6=$(networksetup -getinfo Wi-Fi | grep IPv6: || true)
if echo "$IPV6" | grep -q Off; then
	pass "IPv6 Off for doh-ipv4"
else
	fail "IPv6 not Off: $IPV6"
fi
...
MODE=$(sudo awk -F= '$1=="PROFILE_MODE"{print $2;exit}' /etc/controld/active_profile 2>/dev/null || echo "")
if [[ $MODE == "doh-ipv6" ]]; then
	pass "PROFILE_MODE=doh-ipv6"
else
	fail "expected doh-ipv6 got '$MODE'"
fi
IPV6=$(networksetup -getinfo Wi-Fi | grep IPv6: || true)
if echo "$IPV6" | grep -q Automatic; then
	pass "IPv6 Automatic for doh-ipv6"
else
	warn "IPv6 not Automatic yet: $IPV6"
fi
...
if dig @127.0.0.1 google.com +short +time=3 >/dev/null; then
	pass "DNS resolves after rapid switches"
else
	fail "DNS broken after rapid switches"
fi
