...
Usage: get_prs.sh [options]
  ...
  --details       Fetch reviews + issue comments per PR (slower; richer context)
...
if [[ -n $CONFIG_PATH_RESOLVED ]] && load_bot_authors_from_config "$CONFIG_PATH_RESOLVED"; then
	:
else
	BOT_AUTHORS=(
		"dependabot[bot]"
...
run_legacy_bots_only() {
	...
	for repo in "${REPOS[@]}"; do
		for author in "${BOT_AUTHORS[@]}"; do
			echo "### ${repo} — \`${author}\`"
			echo ""
			if ! json_raw="$(gh pr list --repo "$repo" --author "$author" --state open --limit "$LIMIT" --json \
				number,title,author,isDraft,headRefName,body,createdAt,updatedAt,mergeable,mergeStateStatus,url,statusCheckRollup \
				2>&1)"; then
				printf '%s\n' "$json_raw" >&2
				echo "_gh error._"
